    ...
    return (
        <div className="flex flex-1 flex-col gap-5 p-4 lg:p-6 pb-8">
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600">
                {/* Animated glow orbs */}
                <div className="absolute -top-20 -right-20 size-60 rounded-full bg-fuchsia-500/30 blur-3xl" />
                <div className="absolute -bottom-20 -left-20 size-60 rounded-full bg-violet-400/20 blur-3xl" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-40 rounded-full bg-white/5 blur-2xl" />
                ...
                <div className="relative z-10 p-5 sm:p-8 lg:p-10">
                    ...
                    <div className="flex items-start justify-between mb-6 sm:mb-8">
                        ...
                        <Link to="/app/topup">
                            <div className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/10 border border-white/15 backdrop-blur-md hover:bg-white/20 transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105">
                    ...
                    <div className="flex flex-wrap gap-3">
                        <Link to="/app/generate">
                            <Button className="bg-white text-black hover:bg-white/90 font-semibold rounded-full px-5 h-10 shadow-lg shadow-white/10 gap-2 transition-all duration-300 hover:shadow-xl hover:scale-105">
                                <Sparkles className="size-4" /> Tạo ảnh ngay
                            </Button>
                        </Link>
                        <Link to="/app/templates">
                            <Button variant="outline" className="border-white/20 text-white hover:bg-white/10 rounded-full px-5 h-10 backdrop-blur-sm gap-2 transition-all duration-300 hover:scale-105">
            ...
            <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
                {[
                    { label: "Ảnh đã tạo", value: totalImages, icon: ImageIcon, gradient: "from-violet-500/10 to-purple-500/10", iconColor: "text-violet-400", borderColor: "border-violet-500/10" },
                    { label: "Gems đã dùng", value: totalSpent, icon: Zap, gradient: "from-amber-500/10 to-orange-500/10", iconColor: "text-amber-400", borderColor: "border-amber-500/10" },
                    { label: "Gems hiện có", value: gems, icon: Gem, gradient: "from-emerald-500/10 to-teal-500/10", iconColor: "text-emerald-400", borderColor: "border-emerald-500/10" },
                ].map((s) => (
                    <div key={s.label} className={`relative overflow-hidden rounded-2xl border ${s.borderColor} bg-gradient-to-br ${s.gradient} p-3.5 sm:p-4 transition-all duration-300 hover:scale-[1.02]`}>
                        <div className="flex items-center gap-2.5">
                            <div className={`flex items-center justify-center size-9 sm:size-10 rounded-xl bg-background/50 ${s.iconColor}`}>
                            ...
                            <div className="min-w-0">
                                {loading ? (
                                    ...
                                    <p className="text-lg sm:text-xl font-bold tabular-nums truncate">{s.value.toLocaleString()}</p>
                                )}
            ...
            <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
                {[
                    { to: "/app/generate", label: "Tạo Ảnh", icon: Sparkles, from: "from-violet-600", to_: "to-purple-700" },
                    { to: "/app/templates", label: "Kiểu Mẫu", icon: SwatchBook, from: "from-blue-600", to_: "to-cyan-700" },
                    { to: "/app/library", label: "Thư Viện", icon: Library, from: "from-rose-600", to_: "to-pink-700" },
                ].map((item) => (
                    <Link key={item.to} to={item.to} className="group">
                        <div className={`relative overflow-hidden rounded-2xl aspect-[2/1] sm:aspect-[2.5/1] bg-gradient-to-br ${item.from} ${item.to_}`}>
                            {/* Decorative glow */}
                            <div className="absolute -top-8 -right-8 size-20 rounded-full bg-white/10 blur-2xl group-hover:bg-white/20 transition-all duration-500" />
            ...
            <section className="space-y-3">
                ...
                {loading ? (
                    ...
                    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-500/5 to-fuchsia-500/5 border border-border/50">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
                        {recentImages.map((img) => (
                            <Link key={img.id} to="/app/library" className="group relative aspect-[3/4] rounded-2xl overflow-hidden bg-muted ring-1 ring-border/10 hover:ring-border/30 transition-all duration-300 hover:shadow-lg hover:shadow-black/10">
                                <img
                                    src={img.file_url}
                                ...
                                {img.gems_cost != null && img.gems_cost > 0 && (
                                    <div className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-black/40 backdrop-blur-sm text-[10px] text-white/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <section className="space-y-3">
                ...
                {loading ? (
                    ...
                    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500/5 to-cyan-500/5 border border-border/50">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
                        {templates.map((tpl) => (
                            <Link key={tpl.id} to={`/app/templates/${tpl.slug}`} className="group">
                                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-muted ring-1 ring-border/10 hover:ring-border/30 transition-all duration-300 hover:shadow-lg hover:shadow-black/10">
                                    {tpl.thumbnail ? (
                                        <img
                                            src={tpl.thumbnail}
                                            alt={tpl.name}
